...
export type RemoteDataResult<S = any, F = any> =
    | RemoteDataSuccess<S>
    | RemoteDataFailure<F>;
...
export function isSuccess<S>(
    remoteData: RemoteData<S>,
): remoteData is RemoteDataSuccess<S> {
    return remoteData.status === Status.Success;
}

export function isSuccessAll<S>(
    responses: Array<RemoteData<S>>,
): responses is Array<RemoteDataSuccess<S>> {
    return responses.every(isSuccess);
}

export function isFailure<F>(
    remoteData: RemoteData<any, F>,
): remoteData is RemoteDataFailure<F> {
    return remoteData.status === Status.Failure;
}

export function isFailureAny<F>(
    responses: Array<RemoteData<any, F>>,
): responses is Array<RemoteDataFailure<F>> {
    return responses.some(isFailure);
}

export function isLoadingAny(
    responses: Array<RemoteData>,
): responses is Array<RemoteDataLoading> {
    return responses.some(isLoading);
}

export function isNotAskedAny(
    responses: Array<RemoteData>,
): responses is Array<RemoteDataNotAsked> {
